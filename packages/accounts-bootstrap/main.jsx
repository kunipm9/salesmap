      ...
      <form ref={(ref) => this.form = ref} className={[ "accounts", className ].join(' ')}>
        ...
        { buttons['switchToPasswordReset'] ? (
          <fieldset className="form-group">
            <Accounts.ui.Button className="forget-account" {...buttons['switchToPasswordReset']} />
          </fieldset>
        ) : null }
        <fieldset className="form-group">
          ...
          { buttons['switchToSignIn'] ? (
            <Accounts.ui.Button {...buttons['switchToSignIn']} type="button" className="btn-info ui button" />
          ) : null }
          { buttons['switchToSignUp'] ? (
            <Accounts.ui.Button {...buttons['switchToSignUp']} type="button" className="btn-info ui button" />
          ) : null }
          { buttons['switchToChangePassword'] ? (
            <Accounts.ui.Button {...buttons['switchToChangePassword']} type="button" className="btn-info ui button" />
          ) : null }
          { buttons['switchToSignOut'] ? (
            <Accounts.ui.Button {...buttons['switchToSignOut']} type="button" className="btn-info ui button" />
          ) : null }
        </fieldset>
        <fieldset>
...
console.log("labels.join(' / ')", labels, labels.join(' / '));
      return (
        <div className="or-sep">
          <p style={ style } className={ className }>
            { ` ${ labels.join(' / ') } ${T9n.get('orUse')} ` }
          </p>
        </div>
      );
    }
