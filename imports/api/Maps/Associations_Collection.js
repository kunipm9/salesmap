...
/// Custom - Collection
export const Maps_Associations_Collection = new Mongo.Collection("Maps_Associations");
Maps_Associations_Collection.defaultCollectionHooks();
/// Custom - Collection --

/// Custom - Persister
if (Meteor.isClient) {
  Maps_Associations_Collection.attachPersister();
}
/// Custom - Persister --

const Maps_Associations_Schema = new SimpleSchema({
  _id: {
    optional: true,
    type: String,
    uniforms: {
...
Maps_Associations_Collection.attachSchema(Maps_Associations_Schema, {
  tracker: Tracker
});
/// Custom - AutoForm - schema --

/// Custom - Collection - helpers
Maps_Associations_Collection.dispTitle = function(doc) {
  /// System - Collection - soft delete
  return displayTitle(doc._deleted, doc.name);
  /// System - Collection - soft delete --
};

Maps_Associations_Collection.helpers({
  disp_title() {
    return Maps_Associations_Collection.dispTitle(this);
  }
});
/// Custom - Collection - helpers --
...
Meteor.methods({
  "Maps_Associations.insert": function(Users_Groups_id, data) {
    "use strict";

    setKeyword(data);

    data.Users_Groups_id = Users_Groups_id;
    return Maps_Associations_Collection.insert(data);
  },

  "Maps_Associations.update": function(Users_Groups_id, _id, data) {
    "use strict";

    setKeyword(data);

    /// Custom - Collection - optimistic lock
    let tmp = Maps_Associations_Collection.findOne({ _id: _id });
...
tmp = data;
    ...
    if (tmp.modifiedAt != data.modifiedAt) {
      console.log(
        "Maps_Associations.update tmp.modifiedAt != data.modifiedAt",
        tmp,
        data
      );
    ...
    data.Users_Groups_id = Users_Groups_id;
    Maps_Associations_Collection.update(_id, { $set: data });
  },

  "Maps_Associations.remove": function(Users_Groups_id, _id, data) {
    "use strict";

    /// Custom - Collection - optimistic lock
    const tmp = Maps_Associations_Collection.findOne({ _id: _id });
    if (tmp.modifiedAt != data.modifiedAt) {
      const otherUser = Meteor.users.findOne({ _id: tmp.modifiedBy });
      if (otherUser) {
        throw new Meteor.Error(
    ...
    /// Custom - Collection - soft delete
    // Maps_Associations_Collection.remove({Users_Groups_id: Users_Groups_id, _id: _id});
    Maps_Associations_Collection.update(_id, { $set: { _deleted: _id } });
    /// Custom - Collection - soft delete --
  },

  // eslint-disable-next-line no-unused-vars
  "Maps_Associations.recover": function(Users_Groups_id, _id, editData) {
    "use strict";

    /// Custom - Collection - soft delete
    Maps_Associations_Collection.update(_id, { $set: { _deleted: null } });
    /// Custom - Collection - soft delete --
  }
});
/// Custom - Collection - method
...
if (Meteor.isServer) {
  Meteor.publish("Maps_Associations", function(Users_Groups_id) {
    "use strict";

    console.log("### Meteor.publish('Maps_Associations')", Users_Groups_id);
    return Maps_Associations_Collection.find({ Users_Groups_id: Users_Groups_id });
  });
  ...
  Meteor.startup(function() {
    ...
    /// Custom - Collection - soft delete
    console.log(
      "Maps_Associations_Collection._ensureIndex({ Users_Groups_id: 1, name: 1, _deleted: 1 }, { unique: true });"
    );
    Maps_Associations_Collection._ensureIndex(
...
if (Meteor.isClient) {
  Tracker.autorun(() => {
    ...
    Meteor.subscribe("Maps_Associations", Session.get("Users_Groups_id"));
    console.log(
      "### Meteor.subscribe('Maps_Associations');",
