 ...
 */
export class MapsView_Base extends React.Component {
  "use strict";
  ...
  updateSelector = (key, value) => {
    ...
    Session.set(this.Collection._name + ".List.selector", this.state.selector);
  };
  /// Sys - Collection - selector --
  ...
  loadData() {
    ...
    if (window.$GLOBAL$.__MapView__.requestReDrawMap) {
   ...
   * @memberof MapsView
   */
  getDocsInMapViewSuper = (pathCoordinates, limitMarginX, limitMarginY) => {
    ...
    const worldWidth = this.mapExtent.maxWorld.x - this.mapExtent.minWorld.x;
    const worldHeight = this.mapExtent.maxWorld.y - this.mapExtent.minWorld.y;

    let marginX = worldWidth < limitMarginX ? worldWidth : limitMarginX;
    let marginY = worldHeight < limitMarginY ? worldHeight : limitMarginY;

    console.log("getDocsInMapViewSuper =================", marginX, marginY);
    ...
    for (let key in this.Collection.docs) {
      ...
      if (
        p[0] < this.mapExtent.minWorld.x - marginX ||
        p[0] > this.mapExtent.maxWorld.x + marginX
      ) {
        continue;
      }
      if (
        p[1] < this.mapExtent.minWorld.y - marginY ||
        p[1] > this.mapExtent.maxWorld.y + marginY
      ) {
        continue;
      }
