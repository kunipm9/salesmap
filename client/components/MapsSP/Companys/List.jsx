...
import { displayCompany } from "./utils";
 ...
 * @returns
 */
export const MapsSP_Companys_List_ComponentInfo = mode => {
  ...
  return { title: "MapsSP/Companys/List", path: "/MapsSP/Companys/List" };
};
/// Custom - Role - check permission --

export class _MapsSP_Companys_List extends ListPersistedScroll {
  "use strict";
   ...
   */
  constructor(props) {
    console.log(new Date().getTime(), "Companys List constructor");
    super(props);

    /// Custom - Tabular - link button
    this.detailPath = "/MapsSP/Companys/Detail/";
    /// Custom - Tabular - link button --

    /// Custom - Role - check permission
    this.ComponentInfo = MapsSP_Companys_List_ComponentInfo;
    /// Custom - Role - check permission --
   ...
   */
  componentDidMount() {
    console.log(new Date().getTime(), "Companys List componentDidMount");
   ...
   */
  queryDocs() {
    console.log(new Date().getTime(), "Companys List queryDocs");
    /// Custom - Collection - selector
    const query = {};
   ...
   */
  fetchMoreData = () => {
    console.log(new Date().getTime(), "Companys List fetchMoreData");
    ...
    console.log("Companys fetchMoreData count", count);
    ...
    console.log(
      new Date().getTime(),
      "Companys List fetchMoreData end",
      end - start
    );
  };
  /// Custom - InfiniteScroll --
   ...
   */
  render() {
    console.log(new Date().getTime(), "Companys List render");
    const start = new Date().getTime();

    /// Custom - Collection - subscribe
    if (this.props.loading) {
      console.log(new Date().getTime(), "Companys List render loading...");
      return <span />;
    }
    /// Custom - Collection - subscribe --
    ...
    const end = new Date().getTime();
    console.log(new Date().getTime(), "Companys List render end", end - start);
...
/// Custom - List - tracker
export const MapsSP_Companys_List = withTracker(() => {
  /// Custom - Collection - subscribe
  const handles = [
  ...
  return {
    CollectionList: Maps_Companys_Collection.find({ _deleted: null }).map(b => {
      return b;
    }),
    ComponentInfo: MapsSP_Companys_List_ComponentInfo,
    loading: loading
  };
  /// Custom - Collection - subscribe --
})(_MapsSP_Companys_List);
/// Custom - List - tracker --
