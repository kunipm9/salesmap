 ...
 * @returns
 */
export const MapsSP_Companys_Detail_ComponentInfo = mode => {
  ...
  return { title: "MapsSP/Companys/Detail", path: "/MapsSP/Companys/Detail" };
};
/// Custom - Role - check permission --
 ...
 *
 * @export
 * @class Maps_Companys_View_Detail
 * @extends {Detail}
 */
export class _MapsSP_Companys_Detail extends Update {
  "use strict";

  /**
   *Creates an instance of Maps_Companys_View_Detail.
   * @param {*} props
   * @memberof Maps_Companys_View_Detail
   */
  constructor(props) {
    super(props);
    ...
    /// Custom - Role - check permission
    this.ComponentInfo = MapsSP_Companys_Detail_ComponentInfo;
    /// Custom - Role - check permission --
   ...
   *
   *
   * @memberof MapsSP_Companys_Detail
   */
  displayTab1 = () => {
    this.onChangeTabs("1");
   ...
   *
   * @returns
   * @memberof Maps_Companys_View_Insert
   */
  render() {
    ...
    if (!this.props.doc || !this.props.doc._id) {
      this.props.history.replace("/MapsSP/Companys/List");
      return <React.Fragment />;
    }
    ...
    const panes = [
      ...
      {
        menuItem: `社員 ${this.displayTagNum(this.props.consumersNum)}`,
        render: () => (
          <Tab.Pane
            attached={false}
            style={{ padding: 0, margin: 0, border: "none" }}
 ...
 *
 **/
export const MapsSP_Companys_Detail = withTracker(arg => {
  //TODO
  if (!arg) {
    return;
  }
  ...
  if (loading) {
    return {
      loading: loading,
      doc: {},
      ComponentInfo: MapsSP_Companys_Detail_ComponentInfo
    };
  }
  /// Custom - Collection - subscribe --

  const doc = Maps_Companys_Collection.findOne({ _id: arg.match.params._id });
  if (!doc) {
  ...
  if (doc && doc._id) {
    ...
    const query = { "companys.company.Maps_Company_id": arg.match.params._id };
    consumers = mingo
