...
export class Sel_Introduction extends Sel_BaseNoHist {
  "use strict";
  ...
  componentDidMount() {
    ...
    const tmp = _.get(this.props.doc, "introductions") || [];
    const excludeList = tmp.map(t => t.introduction.Maps_Consumers_id);
   ...
   */
  onClickConfirm = () => {
    console.log(new Date().getTime(), "Sel_Introduction onClickConfirm");

    const doc = this.props.doc;

    if (!doc.introductions) {
      doc.introductions = [];
    }
    if (this.state.selectDoc._id) {
      doc.introductions.push({
        introduction: {
    ...
    this.props.updateDoc(doc, [
      "introductions",
      `introductions.${doc.introductions.length -
        1}.introduction.Maps_Consumers_id`,
      `introductions.${doc.introductions.length - 1}.introduction.relationship`
    ]);
   ...
   * @memberof Sel_Introduction
   */
  onChangeIntroductionRelationship = () => {
      ...
      >
        ...
        <MDBModalBody id="modal-padding">
          <Grid>
            <Grid.Row>
              <Input
                onChange={this.onChangeIntroductionRelationship}
                transparent
   ...
   */
  displayTab2 = () => {
    return this.displayTab2Common("MapsSP_Consumers_Search_Introduction");
  };
}
